...
class Torneos_model extends CI_Model {
    ...
    public function get_tabla_from_torneo($idtorneo) {
        ...
        $query = $this->db->query("SELECT tp.*,(tp.goles_a_favor - tp.goles_en_contra) diff,eq.nombre FROM tfc_dev.tfc_torneo_tabla_posiciones tp join tfc_dev.tfc_equipo eq on tp.idequipo=eq.id where idtorneo=" . $idtorneo . " order by tp.puntos desc,diff desc, tp.goles_a_favor desc");
        return $query->result_array();
    }
